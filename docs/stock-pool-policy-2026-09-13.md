...
**Backpack gap correction:** Stonkfun's listed `pool` can be the original launch pool while its token later trades in another pool. The original `stonkfun-v1` exact-address rule missed these migrated pools. Under `stonkfun-v2`, a DEX Screener pool qualifies when its two exact mints match the launch mint and verified stock quote mint in Stonkfun's public listing, in either orientation. Different pool addresses for that same verified pair each represent a distinct observed pool; duplicate records of one address count once. For example, the CYPHERCAT/CYPH traded pool is distinct from the original listed Stonkfun pool. This still excludes unrelated meme pairs. Stonkfun's token-level volume is not added to pool volume because it may overlap or cover other pools; original launch pools absent from DEX Screener remain unavailable. The top-volume discovery feed and DEX Screener results remain partial, so Sunrise parity is not implied. `stonkfun-v2` separates the new cache and chart observations from the earlier rule.

The original release included a Stonkfun launch pool when the public top-volume feed identified the exact pool address, launch mint and quote mint, and the quote mint matched Float's verified stock registry. DEX Screener supplies observed rolling 24-hour volume and liquidity. Its pool-address endpoint supplements token discovery. Stonkfun's own volume number is **not** added to the DEX Screener number, because these can describe the same trades and currently differ by methodology.

The SPYx-quoted STONK pool `7a8xxAJBELDo6P9dikSYctdw6ce8F4mWr3ahcAD8Ao49` is an example. This is activity trading STONK *against* SPYx, not direct SPYx equity purchases. Another observed pool with the same two verified mints qualifies under v2, while an unlisted lookalike or another memecoin/stock pair does not qualify merely by sharing a ticker. The official discovery feed's first 100 top-volume launches is partial, and DEX Screener can omit or delay pools; neither source establishes full market volume or screens wash trading. If the Stonkfun feed fails, the reviewed settlement/stock pool subset remains available but excludes Stonkfun-only pools for that observation.

Historical chart points are shown only when their policy version matches, so values from different definitions are not compared as if they used the same rule. The new definition applies consistently to all issuers and the public Backpack view.
