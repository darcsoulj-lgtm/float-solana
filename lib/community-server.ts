...
export const authorColumns =
  'm.alias,m.bio,m.avatar_key, CASE WHEN m.show_badge=1 AND m.verified_until>? AND m.suspended=0 THEN m.qualifying_symbol ELSE NULL END badge, CASE WHEN m.show_value_badge=1 AND m.verified_until>unixepoch()*1000 AND m.value_tier_expires_at>unixepoch()*1000 AND m.suspended=0 THEN m.value_tier ELSE NULL END value_tier, m.value_tier_expires_at';
